...
[IC_train,IX] = sort(index(1:trainIndex));
y_train_sort = y_train(IX',:);
y_est_train_sort = y_guess_train{1}(IX',:);

[IC_validate,IX] = sort(index(trainIndex+1:validateIndex));
y_validate_sort = y_validate(IX',:);
y_est_validate_sort = y_guess_validate{minIndex}(IX',:);

[IC_test,IX] = sort(index(validateIndex+1:testIndex));
...
for i=1:dataSize
    if (isempty(find(IC_train==i,1))==0)
        obs_ix(i) = 0;
    elseif (isempty(find(IC_validate==i,1))==0)
        obs_ix(i) = 1;
    elseif (isempty(find(IC_test==i,1))==0)
        obs_ix(i) = 2;
    end
end
...
for i=2:dataSize
    ...
    % [K] = 3\times 2
    if (obs_ix(i)==0) % train
        loc_noise = y_est_train_sort(obv_count,:);
    elseif (obs_ix(i)==1) % validate
        loc_noise = y_est_validate_sort(obv_count,:);
    elseif (obs_ix(i)==2) % test
